...
app.use(async ctx => {
  ...
  // console.log(8, url, ctx.request.header.referer)
  if (/\/\b(\w+)$/.test(url)) {
    // const name = /\/\b(\w+)$/.exec(url)[1]
    // if (/\.(t|j)s$/.test(ctx.request.header.referer)) {
    if (ctx.request.header.referer !== undefined) {
      let p = path.join(__dirname, `/dist${url}.js`)
      if (!fs.existsSync(p)) {
        p = path.join(__dirname, `/dist${url}/index.js`)
      }
      ctx.type = 'application/javascript'
      ctx.body = rewriteImport(fs.readFileSync(p, 'utf8'))
    } else {
      let p = path.join(__dirname, `./src/${url}.html`)
      if (!fs.existsSync(p)) {
        p = path.join(__dirname, `./src/${url}/index.html`)
      }
      ctx.type = 'text/html'
    ...
    const p = path.join(__dirname, '/dist/src' + url.replace(/ts$/, 'js'))
    ctx.type = 'application/javascript'
    ctx.body = rewriteImport(fs.readFileSync(p, 'utf8'))
...
// 模块地址重写
function rewriteImport(content: any) {
  // 把所读取的文件内容的模块,进行更改
  // 把以.或者..开头，.或者..结尾中间的内容进行匹配
  // s1是匹配的部分，s2是分组的内容
  return content.replace(/ from ['"](.*)['"]/g, (s1: string, s2: string) => {
    // 如果是以./ / ../开头的话，则直接进行返回所匹配的内容
    // console.log(60, s2, "-----");
    if (s2.startsWith('./') || s2.startsWith('/') || s2.startsWith('../')) return s1
    // 否则就返回对应modules的对应路径
